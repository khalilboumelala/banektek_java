...
public class DashboardController implements Initializable {
   ...
   public Button add_agent_btn;
    @FXML
    public Button update_agent_btn;
    @FXML
    public Button delete_agent_btn;
    private double x;
    private double y;
    ...
    @FXML
    private TableColumn<Agent, Integer> table_agent_id;
    @FXML
    private TableColumn<Agent, String> table_agent_nom;
    @FXML
    private TableColumn<Agent, String> table_agent_prenom;
    @FXML
    private TableColumn<Agent, String> table_agent_poste;
    @FXML
    private TableColumn<Agent, String> table_agent_matricule;
    @FXML
    private TableColumn<Agent, String> table_agent_email;
    @FXML
    private TextField nom_agent;
    @FXML
    private TextField prenom_agent;
    @FXML
    private TextField poste_agent;
    @FXML
    private TextField email_agent;
    @FXML
    private TextField matricule_agent;
    @FXML
    ...
    public void selectAgent() {
        ...
        if (selectedAgent == null) {
            // Aucun agent sélectionné, rien à faire
            return;
        }

        // Remplir les champs de texte avec les données de l'agent sélectionné
        nom_agent.setText(selectedAgent.getNom());
        prenom_agent.setText(selectedAgent.getPrenom());
        poste_agent.setText(selectedAgent.getPoste());
        email_agent.setText(selectedAgent.getEmail());
    }


    private void loadAgentData() {
        agentData = FXCollections.observableArrayList(agentService.getAll());
    ...
    private void handleAddAgent(ActionEvent event) {
        if (!validateNomPrenom(nom_agent.getText())) {
        ...
        if (!validateNomPrenom(prenom_agent.getText())) {
        ...
        if (poste_agent.getText().isBlank()) {
            setRedText(poste_agent);
            showAlert("Poste");
            return;
        } else {
            resetTextFieldStyle(poste_agent);
        }

        if (matricule_agent.getText().isBlank()) {
            setRedText(matricule_agent);
            showAlert("Matricule");
            return;
        } else {
            resetTextFieldStyle(matricule_agent);
        }

        if (!validateEmail(email_agent.getText())) {
...
// Si tous les champs obligatoires sont remplis, procédez au reste du traitement



        String nom = nom_agent.getText();
        String prenom = prenom_agent.getText();
        String poste = poste_agent.getText();
        String email = email_agent.getText();

        Agent newAgent = new Agent(0, 13,nom, prenom, poste, "", "", email, "", "", "");
        System.out.println("test1");
        if (agentService.add(newAgent)) {
            loadAgentData();
            clearFields();
        }
    ...
    private void handleEditAgent(ActionEvent event) {
        ...
        if (selectedAgent != null) {
            ...
            selectedAgent.setPoste(poste);
            selectedAgent.setEmail(email);

            if (agentService.edit(selectedAgent)) {
                agentTable.refresh();
                clearFields();
            }
        }
    ...
    @FXML
    private void handleDeleteAgent(ActionEvent event) {
        Agent selectedAgent = agentTable.getSelectionModel().getSelectedItem();
        if (selectedAgent != null) {
            ...
            if (result.isPresent() && result.get() == ButtonType.OK) {
                if (agentService.delete(selectedAgent.getId())) {
                    agentData.remove(selectedAgent);
                }
            }
        }
    }
...
//AGENCE
    ...
    @FXML
    private AnchorPane agents_pane;
...
///////////agent/
        table_agent_id.setCellValueFactory(new PropertyValueFactory<>("id"));
        table_agent_nom.setCellValueFactory(new PropertyValueFactory<>("nom"));
        table_agent_prenom.setCellValueFactory(new PropertyValueFactory<>("prenom"));
        table_agent_poste.setCellValueFactory(new PropertyValueFactory<>("poste"));
        table_agent_email.setCellValueFactory(new PropertyValueFactory<>("email"));
