...
public class AgentFXController {
    ...
    public Label fiche_matricule_agent;
    public Label fiche_pass_client;
    double x,y;

    ////:session
    String matricule_agent_toadd;
    String pass_agent_toadd;
...
if(!nom_agent.getText().isBlank() && !prenom_agent.getText().isEmpty()){
    matricule_agent_toadd=Agent.generateMatricule(nom_agent.getText(),prenom_agent.getText());
    pass_agent_toadd=Agent.generateStrongPassword(8);
    fiche_matricule_agent.setText(matricule_agent_toadd);
    fiche_pass_client.setText(pass_agent_toadd);
    ...
    private void handleAddAgentButtonAction(ActionEvent event) {
        ...
        String passwordhashed = PasswordHasher.hashPassword(pass_agent_toadd);
        // Création de l'objet Agent
        Agent agent = new Agent(0, selectedAgenceId, nom, prenom, poste, matricule_agent_toadd, passwordhashed, email, numero, "agents_photos/"+fileName_photo, "");
    ...
    public void remplirChampsAgentEdit(Agent agent) {
        ...
        Agence agence_edit=AgenceService.getInstance().get(agent.getIdAgence());
...
// parts[1] contiendra l'ID de l'agence sélectionnée
       ...
       String username =   Agent.generateMatricule(nom, prenom);
        ...
        Agent agent = new Agent(id_agent_to_edit, selectedAgenceId, nom, prenom, poste, username, password, email, numero, "", "");
