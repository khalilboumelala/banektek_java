...
private Button articles_btn;
    ...
    public void disactivateAllPanes(){
        ...
        agents_btn.setStyle("-fx-background-color:linear-gradient(to bottom right , rgba(121,172,255,0.2),  rgba(255,106,239,0.2))");
  ...
  public void  ActivateThisAnchor(AnchorPane AnchorTest,Button ButtonTest){
    ...
    public void activateAnchorPane(){
        ...
        agents_btn.setOnMouseClicked(mouseEvent -> {
            ActivateThisAnchor(agents_pane,agents_btn);
...
private AnchorPane comptes_pane;
    ...
    FXMLLoader loaderClient= new FXMLLoader(getClass().getResource("DashFX.fxml"));
    Pane ClientLoadedPane;

    FXMLLoader loaderCompte= new FXMLLoader(getClass().getResource("Comptes.fxml"));
    Pane CompteLoadedPane;

    FXMLLoader loaderCarte= new FXMLLoader(getClass().getResource("Cartes.fxml"));
    Pane CarteLoadedPane;
    FXMLLoader loaderVirement= new FXMLLoader(getClass().getResource("VirementTypeVirement.fxml"));
    Pane VirementLoadedPane;
    FXMLLoader loaderTransaction= new FXMLLoader(getClass().getResource("transaction.fxml"));
    Pane TransactionLoadedPane;

    FXMLLoader loaderCredit= new FXMLLoader(getClass().getResource("credits.fxml"));
    ...
    public void initializeAllFxml(){
        ...
        try {
            ...
            ////////agent ///////////////////////
             AgentLoadedPane = loaderAgent.load();
            agents_pane.getChildren().add(AgentLoadedPane);
            ///////////////CLIENT
            ClientLoadedPane = loaderClient.load();
            clients_pane.getChildren().add(ClientLoadedPane);
            ///////////////Compte
            CompteLoadedPane = loaderCompte.load();
            comptes_pane.getChildren().add(CompteLoadedPane);
            ///////////////Carte
            CarteLoadedPane = loaderCarte.load();
            cartes_pane.getChildren().add(CarteLoadedPane);
            ///////////////Virement
            VirementLoadedPane = loaderVirement.load();
            virements_pane.getChildren().add(VirementLoadedPane);
            ///////////////Transaction
            TransactionLoadedPane = loaderTransaction.load();
            transaction_pane.getChildren().add(TransactionLoadedPane);
            ////////////credit
            CreditLoadedPane = loaderCredit.load();
            credit_pane.getChildren().add(CreditLoadedPane);
...
@FXML
    ...
    @FXML
    public void generateAIarticle() {

        String userInput = article_titre_input.getText(); // Assuming you have a TextField `article_titre_input`
        ArticleGeneration task = new ArticleGeneration(userInput,process,builder);

        // Bind task's message property to update UI progressively (optional)
        task.messageProperty().addListener((obs, oldMessage, newMessage) -> {
            Platform.runLater(() -> article_contenu_input.appendText(newMessage + "\n"));
        });

        // When task is completed, update the TextArea with full content
        task.setOnSucceeded(event -> article_contenu_input.setText(task.getValue()));

        // Handle task failure
        task.setOnFailed(event -> article_contenu_input.setText("Failed to generate article. " + task.getException().getMessage()));

        articlegenerationtaskprogressview.getTasks().add(task);
    ...
    @FXML
    public void generateAIarticle() {
        String topic = article_titre_input.getText(); // Get the topic from the article_titre_input TextField

        // Send request to the AI content writer API
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    ...
                    .header("X-RapidAPI-Host", "ai-content-writer.p.rapidapi.com")
                    .method("POST", HttpRequest.BodyPublishers.ofString("topic=" + URLEncoder.encode(topic, StandardCharsets.UTF_8)))
                    .build();

            HttpResponse<String> response = HttpClient.newHttpClient().send(request, HttpResponse.BodyHandlers.ofString());

            // Set the generated text to the article_contenu_input TextField
            article_contenu_input.setText(response.body());
